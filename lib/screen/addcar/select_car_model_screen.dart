...
class _SelectCarModelScreenState extends State<SelectCarModelScreen> {
  late Future<List<CarModel>> _carModelNamesFuture;

  @override
  void initState() {
    super.initState();
    _carModelNamesFuture = _getCarModel();
  }

  Future<List<CarModel>> _getCarModel() async {
    try {
      var response = await RemoteServices.getCarModel(
          widget.jwtToken, Constants.carModelId);
      if (response.statusCode == 200) {
        List<dynamic> jsonResponse = jsonDecode(response.body)['data'];
        List<CarModel> carModels = [];
        for (var item in jsonResponse) {
          carModels.add(CarModel(item['id'], item['name']));
        }
        return carModels;
      } else {
